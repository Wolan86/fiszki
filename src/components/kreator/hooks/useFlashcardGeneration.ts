...
import type { FlashcardDto, UpdateFlashcardCommand, CreateFlashcardCommand, CreateSourceTextResponse, UnsavedFlashcardDto } from "@/types";
import type { 
  ApiErrorResponse, 
  FlashcardViewModel, 
...
export const useFlashcardGeneration = (): UseFlashcardGenerationResult => {
  ...
  const handleSaveFlashcard = async (id: string): Promise<void> => {
    try {
      ...
      setFlashcards(prev => prev.map((card: FlashcardViewModel) => 
        card.id === id 
          ? { 
              ...savedFlashcard, 
