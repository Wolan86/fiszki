...
export const FlashcardGrid: React.FC<FlashcardGridProps> = ({ flashcards, onEdit, onDelete, loading = false }) => {
  ...
  if (flashcards.length === 0) {
    return (
      <div className="text-center py-12" data-testid="flashcard-grid-empty">
        <div className="text-gray-500 text-lg mb-2">Brak fiszek do wyświetlenia</div>
        <p className="text-gray-400">Rozpocznij naukę tworząc swoje pierwsze fiszki</p>
      </div>
    );
  }
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" data-testid="flashcard-grid">
        {flashcards.map((flashcard) => (
          <FlashcardViewItem key={flashcard.id} flashcard={flashcard} onEdit={onEdit} onDelete={onDelete} />
        ))}
      </div>
