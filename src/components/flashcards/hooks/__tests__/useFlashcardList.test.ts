...
describe('useFlashcardList', () => {
  ...
  describe('editFlashcard', () => {
    ...
    it('should trim content before sending', async () => {
      ...
      // Act
      await act(async () => {
        await result.current.editFlashcard(
          'test-id',
          '  front content  ',
          '  back content  '
        );
    ...
    it('should handle 404 error correctly', async () => {
      ...
      await act(async () => {
        await expect(
          result.current.editFlashcard('non-existent', 'front', 'back')
        ).rejects.toThrow('Fiszka nie została znaleziona');
      });
    });
    ...
    it('should handle 400 validation error', async () => {
      ...
      await act(async () => {
        await expect(
          result.current.editFlashcard('test-id', '', 'back')
    ...
    it('should handle 400 error without message', async () => {
      ...
      await act(async () => {
        await expect(
          result.current.editFlashcard('test-id', 'front', 'back')
        ).rejects.toThrow('Błąd walidacji danych');
      });
    });
  ...
  describe('cleanup and memory management', () => {
    ...
    it('should cancel previous search when new search is triggered', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledTimes(1);
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining('search=second'),
        expect.any(Object)
      );
    });
  });
