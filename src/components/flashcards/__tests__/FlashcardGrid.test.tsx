...
vi.mock('../FlashcardViewItem', () => ({
  FlashcardViewItem: ({ flashcard, onEdit, onDelete }: any) => (
    <div 
    ...
    >
      ...
      <button 
        ...
        data-testid={`edit-btn-${flashcard.id}`}
      >
        Edit
      </button>
      <button 
        onClick={() => onDelete(flashcard.id)}
        data-testid={`delete-btn-${flashcard.id}`}
      >
        Delete
      </button>
    </div>
...
describe('FlashcardGrid', () => {
  ...
  describe('data handling and performance', () => {
    ...
    it('should handle flashcards with very long content', () => {
      ...
      render(
        <FlashcardGrid
          flashcards={longContentFlashcards}
          onEdit={mockOnEdit}
          onDelete={mockOnDelete}
        />
      );
  ...
  describe('layout and visual consistency', () => {
    it('should maintain consistent grid structure with different content lengths', () => {
      ...
      const mixedContentFlashcards = [
        ...
        {
          id: '3',
          front_content: 'Very long content that would definitely span multiple lines and possibly overflow if not handled properly with appropriate text wrapping and container sizing',
          back_content: 'Very long back content that also spans multiple lines and needs to be handled gracefully within the grid layout system',
          created_at: '2023-01-03T00:00:00Z',
          updated_at: '2023-01-03T00:00:00Z',
          accepted: null,
          creation_type: null,
      ...
      render(
        <FlashcardGrid
          flashcards={mixedContentFlashcards}
          onEdit={mockOnEdit}
          onDelete={mockOnDelete}
        />
      );
