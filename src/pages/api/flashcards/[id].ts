...
import { updateFlashcard, updateFlashcardSchema, deleteFlashcard } from "../../../lib/services/flashcard.service";
 ...
 *
 * Deletes an existing flashcard owned by the authenticated user.
 *
 * @description
 * This endpoint allows authenticated users to delete their flashcards.
 * Only the flashcard owner can delete their flashcards.
 * Once deleted, the flashcard cannot be recovered.
 *
 * @authentication Required - Supabase session
 *
 * @params
 * - id: UUID of the flashcard to delete
 *
 * @responses
 * - 204: Flashcard deleted successfully (no content)
 * - 401: Authentication required (no valid session)
 * - 404: Flashcard not found (doesn't exist or doesn't belong to user)
 * - 500: Internal server error (database or unexpected errors)
 *
 ...
 * ```
 */
export async function DELETE({ request, cookies, params }: APIContext) {
  const timer = new RequestTimer(`DELETE /api/flashcards/${params.id}`);
  ...
  ApiLogger.debug("Flashcard delete request", {
    userId: session.user.id,
    flashcardId,
  });

  try {
    ApiLogger.info("Deleting flashcard", {
      userId: session.user.id,
      flashcardId,
    });

    // Delete flashcard using service
    await deleteFlashcard(supabase, flashcardId, session.user.id);

    ApiLogger.info("Flashcard deleted successfully", {
      flashcardId,
      userId: session.user.id,
    });
