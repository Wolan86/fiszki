...
export class OpenRouterService {
   ...
   */
  public async chat(input: string | ChatMessage[], options?: ChatOptions): Promise<ChatResponse> {
    try {
  ...
  private formatMessages(input: string | ChatMessage[], systemMessage?: string): ChatMessage[] {
   ...
   */
  private createFlashcardPrompt(text: string, count: number, customPrompt?: string): string {
    if (customPrompt) {
          ...
          return [
            {
              ...
              back_content:
                "The AI service was unable to generate flashcards from your text. Please try again or use a different text.",
            },
          ];
        }
      }
      ...
      return [
        {
          front_content: "Error generating flashcards",
          back_content:
            "The AI service was unable to generate flashcards from your text. Please try again with different text or settings. Error: " +
  ...
  private async makeRequest<T>(endpoint: string, data: Record<string, unknown>, options?: RequestOptions): Promise<T> {
